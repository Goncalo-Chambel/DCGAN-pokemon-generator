...
trained_models_folder = "Aux_models/"
generated_images_folder = "aux_images/"
'''
for x in dataset:
    plt.axis("off")
    plt.figure(1)
...
generator = tf.keras.Sequential(
    [
        ...
        tf.keras.layers.Conv2DTranspose(64 * 2, kernel_size=4, strides=2, padding='same',
                                     kernel_initializer=tf.keras.initializers.RandomNormal(mean=0.0, stddev=0.02),
                                     use_bias=False),
        tf.keras.layers.BatchNormalization(momentum=0.1, epsilon=0.8, center=1.0, scale=0.02),
        tf.keras.layers.ReLU(),

        tf.keras.layers.Conv2DTranspose(64 * 4, kernel_size=4, strides=2, padding='same',
                                     kernel_initializer=tf.keras.initializers.RandomNormal(mean=0.0, stddev=0.02),
                                     use_bias=False),
        tf.keras.layers.BatchNormalization(momentum=0.1, epsilon=0.8, center=1.0, scale=0.02),
        tf.keras.layers.ReLU(),

        tf.keras.layers.Conv2DTranspose(64 * 8, kernel_size=4, strides=2, padding='same',
                                     kernel_initializer=tf.keras.initializers.RandomNormal(mean=0.0, stddev=0.02),
                                     use_bias=False),
        tf.keras.layers.BatchNormalization(momentum=0.1, epsilon=0.8, center=1.0, scale=0.02),
        tf.keras.layers.ReLU(),
...
generator.compile(generator_optimizer, loss=binary_cross_entropy)
discriminator.compile(discriminator_optimizer, loss=binary_cross_entropy)
gan = Sequential([generator, discriminator])
gan.compile(generator_optimizer, loss=binary_cross_entropy)
...
def train(dataset, epochs):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 100 == 0:
            discriminator.save(trained_models_folder + "Discriminator_epoch_%d" % (epoch + 1))
            generator.save(trained_models_folder + "Generator_epoch_%d" % (epoch + 1))
